...
emap = {1: 'Angry',2: 'Disgust',3: 'Fear',4: 'Happy',5: 'Neutral',6: 'Sad',7: 'Surprise',}
subdata = test_data.copy()
y = subdata['Label']
x = torch.Tensor(subdata[feature_ALL].values).float()
y_hat = net(x)[:,0:7]
y_hat = torch.max(F.softmax(y_hat,dim=1),1)[1].numpy()
...
for i in range(7):
    print('Emotion: '+emap[i+1])
    print('precision:{}, recall:{}, specificity:{}'.format(precision[i],recall[i],specificity[i]))
